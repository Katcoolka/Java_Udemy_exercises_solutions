package com.udemy.exercise24.flour.pack.problem;

public class FlourPacket {

	public static void main(String[] args) {

		System.out.println("we can pack 1 big, 0 small, into 4 goal package: " + canPack(1, 0, 4));
		System.out.println("we can pack 1 big, 0 small, into 5 goal package: " + canPack(1, 0, 5));
		System.out.println("we can pack 0 big, 5 small, into 4 goal package: " + canPack(0, 5, 4));
		System.out.println("we can pack 2 big, 2 small, into  11 goal package: " + canPack(2, 2, 11));
		System.out.println("we can pack -3 big, 2 small, into 12 goal package: " + canPack(-3, 2, 12));

	}

	public static boolean canPack(int bigCount, int smallCount, int goal) {

		if ((bigCount < 0) || (smallCount < 0) || (goal < 0)) { //If any of the parameters are negative, return false.
			return false;
		}

		if ((5 * bigCount) + (1 * smallCount) < goal) { //If the sum is greater than goal, ensure that only full bags are used towards the goal amount.Supply of flour has to be bigger than goal .
			return false;
		}
		return (goal % 5 <= smallCount); //it is ok to have more 1kg packages than goal
