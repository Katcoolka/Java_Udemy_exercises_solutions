 ...
 * 
 * - setPrevious(), takes a ListItem and sets it as its leftLink, then it
 * returns leftLink.
 * 
 * - compareTo(), takes a ListItem and compares it to the ListItem that called
 * this method. Use value from ListItem for comparison. If this value is greater
 * than the value that was passed in, then it should return a number greater
 * than zero. If vice versa, then it should return a number less than zero, and
 * zero if equal.
 * 
 ...
 * 
 * - A constructor that takes a ListItem and initialises the field root with the
 * newly passed in parameter.
 * 
 * - And four methods:
 * 
 * - getRoot(), getter for root.
 * 
 * - addItem(), takes a ListItem and returns true if it was added successfully
 * or false otherwise. If the item is already present, it doesn't get added. Use
 * compareTo() to place the item in its proper order.
 * 
 * - removeItem(), takes a ListItem and returns true if it was removed
 * successfully or false otherwise.
 * 
 * - traverse(), takes the root as an argument and does not return anything. If
 * the root is null it prints out: The list is empty, otherwise print each value
 * on a separate line.
 * 
 ...
 * - It has four methods:
 * 
 * - getRoot(), addItem(), removeItem() and traverse() which are package-private
 ...
 * parent. It doesn't return anything and is declared as private. Call this
 * method from removeItem() when the item is found.
 * 
 * - traverse(), takes the root as an argument and does not return anything. It
 * uses recursion to visit all the branches in the tree (Inorder). Print each
 * value on a seperate line.
 * 
 ...
 * 
 * If the item to be added is less than the current item in the tree, add the
 * item before the current item (i.e., make the previous item's "next" refer to
 * the new item, and the new item's "next" refer to the current item).
 * 
 * If the item to be added is greater than the current item, move onto the next
 * item and compare again (if there is no next item, then that is where the new
 * item belongs).
 * 
