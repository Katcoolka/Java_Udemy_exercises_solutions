package com.udemy.exercise26.diagonal.star;
...
public class DiagonalStar {
	...
	public static void printSquareStar(int number) {
		if (number < 5) {
			...
			return; //stop if this condition is met.
		}
		for (int i = 0; i < number; i++) {  //i represents row
			for (int j = 0; j < number; j++) { //j represents column
				if ((i == 0) || (j == 0) || (i == j) || i == (number - 1) || j == (number - 1) || i + j == number - 1) {
					System.out.print("*"); // print("*") just prints the star, but doesn't go to the next line.
				} else {
					System.out.print(" "); // empty space between stars
				}
 ...
 * Explanation:
 ***** 
 * 5 stars ** 2 stars space 2 stars * * 1 star space 1 star space 1 star ** 2
 * stars space 2 stars 5 stars
 * 
 * 
 * EXAMPLE 2
 * 
